...
l2_cost = tf.contrib.layers.l2_regularizer(weight_decay)(networks.all_params[0]) + \
          ...
          tf.contrib.layers.l2_regularizer(weight_decay)(networks.all_params[26]) + \
...
lr2 = tf.train.exponential_decay(learning_rate*10,
                                ...
                                decay_rate,
                                staircase=True)
var_list1 = networks.all_params[:50] 
var_list2 = networks.all_params[50:]
...
if start_step>0:
  load_params = tl.files.load_npz(name='%s/%s_birnn_model_epoch_%d.npz'%(model_prefix, dataset_name, start_epoch))
  ...
  pretrained_model = './pretrained_models/pretrained_model_for_isogr_depth_fintuning.npz'
...
for epoch in range(start_epoch, n_epoch):
  ...
  for _,_ in tl.iterate.minibatches(X_tridx, 
    ...
    _,loss_value,lr_value,acc,pre = sess.run([train_op,cost,lr,networks_accu,networks_y_op], feed_dict=feed_dict)
  ...
  tl.files.save_npz(networks.all_params, 
                    name='%s/%s_birnn_model_epoch_%d.npz'%(model_prefix, dataset_name, epoch+1), 
                    sess=sess)
  print("Model saved in file: %s_birnn_model_epoch_%d.npz" %(dataset_name, epoch+1))
  log.writelines(['Model saved in file: %s_birnn_model_epoch_%d.npz'%(dataset_name, epoch+1), 
                  '\n'])
  log.flush()
  ...
  for X_indices, y_label_t in tl.iterate.minibatches(X_teidx, 
    ...
    _,accu_value = sess.run([prediction_y_op, prediction_accu], feed_dict=feed_dict)
