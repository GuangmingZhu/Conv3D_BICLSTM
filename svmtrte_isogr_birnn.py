...
valid_feats = np.zeros((len(y_test), 12288), dtype=np.float32)
valid_label = np.zeros((len(y_test), 1), dtype=np.float32)
...
for idx, line in enumerate(f_lines):
  linetxt = '%s %s %d\n' %(line.split(' ')[0], line.split(' ')[1], prediction_values[idx]+1)
  f.write(linetxt)
f.close()
