...
def prepare_devisign_rgb_data(image_info):
  ...
  if change_background:
    average_values = [120,101,95]
    bgfile = '/ssd/gmzhu/DEVISIGN_L/BGS1000/bg_%04d.bmp' % random.randint(1,1000)
    bgimage = imread(bgfile)
  else:
    average_values = [100,97,95]
  ...
  for idx in xrange(0, output_frame_cnt):
    ...
    if change_background:
      mask_file = '%s/%04d.bin' %(video_path, rand_frames[idx])
      mask_data = np.fromfile(mask_file, dtype=int)
      for bg in xrange(0, len(mask_data)):
        lp = mask_data[bg] & 0x3FF
        sp = (mask_data[bg] >> 10) & 0x7FF
        ep = (mask_data[bg] >> 21) & 0x7FF
        image[lp,sp:ep,:] = bgimage[lp,sp:ep,:]
...
def prepare_devisign_depth_data(image_info):
  video_path = image_info[0]
  video_frame_cnt = image_info[1]
  output_frame_cnt = image_info[2]
